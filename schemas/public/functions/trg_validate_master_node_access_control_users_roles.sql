CREATE OR REPLACE FUNCTION public.trg_validate_master_node_access_control_users_roles()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    IF NEW.ref_master_table_access_control_users_roles_in_record_id IS NULL THEN

        RAISE EXCEPTION 'ref_master_table_access_control_users_roles_in_record_id cannot be NULL';
    ...
    WHERE mac.in_record_id = NEW.ref_master_table_access_control_users_roles_in_record_id;
    ...
    IF NOT FOUND THEN
        ...
        RAISE EXCEPTION
            ...
            'Invalid parent access control id %, no record found in master_table_access_control_users_roles',

            NEW.ref_master_table_access_control_users_roles_in_record_id;
    ...
    IF v_node_table_id IS DISTINCT FROM v_parent_table_id THEN
        ...
        RAISE EXCEPTION
            ...
            'Node access invalid: node % belongs to master_table %, but parent roles access control % belongs to master_table %',
    ...
    LOOP
        ...
        IF NOT (v_perm = ANY(v_parent_permissions)) THEN
            ...
            RAISE EXCEPTION
                ...
                'Node access invalid: permission % is not allowed because parent table permissions do not include it (roles parent access control %)',
