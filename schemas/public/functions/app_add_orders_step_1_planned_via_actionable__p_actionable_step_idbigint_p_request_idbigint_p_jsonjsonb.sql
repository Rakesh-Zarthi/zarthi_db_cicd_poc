CREATE OR REPLACE FUNCTION public.app_add_orders_step_1_planned_via_actionable(p_actionable_step_id bigint, p_request_id bigint, p_json jsonb)
...
IF p_actionable_step_id IS NULL THEN
    ...
    RAISE EXCEPTION
        ...
        '[STEP_1_PLANNED_VIA][VALIDATION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_created_by IS NULL THEN
    ...
    RAISE EXCEPTION
        ...
        '[STEP_1_PLANNED_VIA][SESSION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_actionable_id IS NULL THEN
    ...
    RAISE EXCEPTION
        ...
        '[STEP_1_PLANNED_VIA][STEP_RESOLVE][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_step_no <> 1 THEN
    ...
    RAISE EXCEPTION
        ...
        '[STEP_1_PLANNED_VIA][STEP_VALIDATE][ERROR] run_id=% txid=% reason=% ts=%',

        v_run_id, v_txid,

        'Only Step 1 can be planned through this API',
...
IF v_step_status <> 'Planned' THEN
    ...
    RAISE EXCEPTION
        ...
        'Step 1 must be Planned. Current status = ' || v_step_status,
