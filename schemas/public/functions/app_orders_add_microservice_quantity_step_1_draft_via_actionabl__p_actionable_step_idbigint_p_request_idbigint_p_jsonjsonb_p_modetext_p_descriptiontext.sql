CREATE OR REPLACE FUNCTION public.app_orders_add_microservice_quantity_step_1_draft_via_actionabl(p_actionable_step_id bigint, p_request_id bigint, p_json jsonb, p_mode text DEFAULT 'DRAFT'::text, p_description text DEFAULT NULL::text)
...
IF p_actionable_step_id IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_1_DRAFT_VIA][VALIDATION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_created_by IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_1_DRAFT_VIA][SESSION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_actionable_id IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_1_DRAFT_VIA][STEP_RESOLVE][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_step_no <> 1 THEN
    ...
    RAISE EXCEPTION '[STEP_1_DRAFT_VIA][STEP_VALIDATE][ERROR] run_id=% txid=% reason=% ts=%',

        v_run_id, v_txid, 'Only Step 1 can be drafted through this API', now();
