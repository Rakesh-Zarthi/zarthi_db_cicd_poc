CREATE OR REPLACE FUNCTION public.app_orders_add_microservice_quantity_step_3_draft_v_0_0_1(p_actionable_step_id bigint, p_request_id bigint, p_response text DEFAULT NULL::text, p_json jsonb DEFAULT NULL::jsonb)
...
IF p_actionable_step_id IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_3_DRAFT][VALIDATION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_created_by IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_3_DRAFT][SESSION][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_actionable_id IS NULL THEN
    ...
    RAISE EXCEPTION '[STEP_3_DRAFT][STEP_RESOLVE][ERROR] run_id=% txid=% reason=% ts=%',
...
IF v_step_no <> 3 THEN
    ...
    RAISE EXCEPTION '[STEP_3_DRAFT][STEP_VALIDATE][ERROR] run_id=% txid=% reason=% ts=%',

        v_run_id, v_txid, 'Only Step 3 can be drafted through this', now();

END IF;



IF v_step_status NOT IN ('Open', 'Draft') THEN

    RAISE EXCEPTION '[STEP_3_DRAFT][STEP_VALIDATE][ERROR] run_id=% txid=% reason=% ts=%',

        v_run_id, v_txid, 'Step 3 must be Open or Draft. Current status = ' || v_step_status, now();
