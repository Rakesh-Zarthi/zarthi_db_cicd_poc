CREATE OR REPLACE FUNCTION public.trg_validate_master_access_control_chain()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    --    Prevent inserting/updating a non-root row unless root exists

    --    for that service SKU (excluding current row on update).
    ...
    IF NEW.is_root IS NOT TRUE THEN
        ...
        IF NOT EXISTS (
            ...
            WHERE mac.ref_services_sku_in_record_id = NEW.ref_services_sku_in_record_id
            ...
            RAISE EXCEPTION
                ...
                'Access config invalid: services_sku % has no root configured. Create root entry first.',
    ...
    IF NEW.is_root IS TRUE
       ...
       AND NOT ('Select'::public.dropdown = ANY(NEW.permission)) THEN
        ...
        RAISE EXCEPTION
            ...
            'Root config invalid: Select permission is mandatory for root table (services_sku %)',
    ...
    LOOP
        ...
        PERFORM public.validate_master_access_chain(
