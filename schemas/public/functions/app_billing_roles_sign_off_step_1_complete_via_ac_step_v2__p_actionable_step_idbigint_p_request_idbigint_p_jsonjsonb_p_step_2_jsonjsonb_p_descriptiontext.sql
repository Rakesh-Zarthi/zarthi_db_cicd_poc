CREATE OR REPLACE FUNCTION public.app_billing_roles_sign_off_step_1_complete_via_ac_step_v2(p_actionable_step_id bigint, p_request_id bigint, p_json jsonb, p_step_2_json jsonb, p_description text DEFAULT NULL::text)
...
IF v_sign_off_type = 'Per Hour' THEN
    ...
    IF v_available_qty < v_quantity THEN
        ...
        RAISE EXCEPTION
            ...
            'Requested quantity % exceeds available Delivery In Progress quantity (%) for task %',
...
END IF;
    ...
    JOIN public.actionables a ON a.in_record_id = ast.ref_actionables_in_record_id
    ...
    IF v_actionable_id IS NULL THEN
        ...
        RAISE EXCEPTION 'Invalid Step 1';
    ...
    IF v_request_subject <> p_request_id THEN
        ...
        RAISE EXCEPTION 'Step does not belong to supplied request';
    ...
    IF v_step_status NOT IN ('Draft', 'Planned') THEN
        ...
        RAISE EXCEPTION 'Step 1 must be Draft or Planned';
    ...
    SET status = 'Open',step_metadata = p_json
    ...
    -- Update Actionable Description

    ------------------------------------------------------------------

    UPDATE public.actionables

    SET actionable_description =

        CASE

            WHEN COALESCE(TRIM(p_description), '') <> '' THEN p_description
...
END;
    ...
    v_desc := ts ->> 'description';
            ...
            -- Insert timesheet and map usage using request owner as professional
...
FROM public.app_sign_off_create_timesheet_v_0_0_1(
    ...
    v_existing_timesheet_id
            ...
            v_total_calculated_hours := v_total_calculated_hours + v_calculated_hours;

        END LOOP;



        -- Validate total hours match requested quantity
